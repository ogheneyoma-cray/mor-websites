 ...
 * theme activation):
 *  - Import Services: creates 20 real WooCommerce products via CRUD
...
require_once MOR_THEME_DIR . '/inc/data/services.php';
...
function mor_render_import_page() {
	...
	<div class="wrap">
		...
		<?php if ( mor_woocommerce_is_missing() ) : ?>
			<div class="notice notice-error"><p><?php esc_html_e( 'WooCommerce must be active before importing services.', 'mor-websites' ); ?></p></div>
			</div>
			<?php return; ?>
		<?php endif; ?>
...
function mor_handle_import_services() {
	...
	foreach ( $catalog as $service ) {
		$existing_id = wc_get_product_id_by_sku( $service['sku'] );
		if ( $existing_id ) {
			$skipped++;
			continue;
		}
		...
		$product->set_sku( $service['sku'] );
		$product->set_regular_price( (string) $service['price'] );
		$product->set_description( $service['description'] );
		$product->set_short_description( wp_trim_words( $service['description'], 25 ) );
		$product->set_status( 'publish' );
		$product->set_catalog_visibility( 'visible' );
		$product->set_virtual( true ); // service, not a shippable physical good.
		...
		$image_path = MOR_THEME_DIR . '/assets/images/services/' . $service['image'];
		$attachment_id = mor_sideload_local_image( $image_path, $service['name'] );
		if ( $attachment_id ) {
	...
	set_transient(
		...
		sprintf(
			...
			__( 'Import complete: %1$d service(s) created, %2$d already existed and were skipped.', 'mor-websites' ),
			$created,
			$skipped
		),
		60
